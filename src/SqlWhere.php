    ...
    private function setStamments() {
        ...
        foreach ($this->clauses as $key=>$clause)
        {
            if(preg_match($pattern, $clause, $match[$key]) == 1)
            {
                ...
                {
                    throw new \Exception("FALHA - setWhere(): [{$clause}] Cl·usula n„o corresponde a um campo da tabela");
                }
            }
            
            if(empty($match[$key]))
            {
                throw new \Exception("FALHA - setWhere(): [{$clause}] String n„o corresponde a uma cl·usula v·lida");
            }
        }
    }
